...
import type { IProps } from './RadioButtonList.d';
...
// useField for validation and value handling
...
<template>
  <div :class="wrapperClass ? wrapperClass : 'dm-field'">
    ...
    <div :class="props.itemsWrapperClass ? props.itemsWrapperClass : 'dm-field'">
      ...
      >
        <VRadioButton
          :input-id="`radio-${item[props.idKey] || item.id || index}`"
          ...
          :is-single="false"
        />
        <label :for="`radio-${item[props.idKey] || item.id || index}`">{{ typeof item === 'object' ? item[props.valueKey] : item }}</label>
      </div>
    </div>
    <slot name="helper">
      <small
